...
from src.core.kinematic.pink.aider_ik import AiderPinkSolver
...
class AiderAdapter:
    ...
    def update_arm_angles(self, arm: str, ik_angles: np.ndarray,
        ...
        if override_wrist:
            ...
            # 2026-09-08 定论: 系统语义 arm6(index5)=俯仰、arm7(index6)=偏航
            #    （settings.WRIST_FLEX_INDEX=5 / WRIST_YAW_INDEX=6、URDF、control_loop 一致）。
            #    真机电机 id15/26=俯仰→arm6, id16/25=偏航→arm7；
            #    servo_ids.yaml 的 arm6/arm7 id 已按此修正（id 物理功能不变，只换关节名）。
            if len(angles) >= 5:
                angles[4] = wrist_roll    # arm5 (Z轴, 前臂旋前/旋后)
            if len(angles) >= 6:
                angles[5] = wrist_flex    # arm6 = 手腕俯仰 (id15 / id26)
            if len(angles) >= 7:
                angles[6] = wrist_yaw     # arm7 = 手腕偏航 (id16 / id25)
        if len(angles) >= 8:
    ...
    def apply_gripper_from_trigger(self, arm: str, trigger_value: float) -> None:
        """根据 VR 扳机值 (0~1) 设置夹爪目标角度（钳制到软限位）。

        映射：扳机 0 → 张开 0°，扳机 1 → 闭合 -90°（角度完整可控，与力控改造前一致）。

        真机扭矩由驱动层自适应提供（见 RobStrideOfficialDriver.set_gripper_adaptive）：
        能跟上目标角度 → 用默认小扭矩；跟不上（抓到物体/被挡）→ 逐步提高到峰值扭矩；
        峰值持续过久 → 回落到持续安全力矩。既抓得稳，又不会过流失能。
        """
        gripper_angle = -float(np.clip(trigger_value, 0.0, 1.0)) * 90.0
        limits = self._soft_arm_limits(arm)
    ...
    def build_hardware_actions(self, servo_ids: dict, servo_ports: dict = None,
        ...
        actions = {"position_commands": [], "speed_commands": [],
                   "torque_commands": [], "gripper_commands": []}
        ...
        # --- 自适应扭矩夹爪：arm8 走专用通道（保留位置环 + 按需扭矩）---
        #     普通位置模式顶到机械限位会堵转过载(STALL_OVERLOAD)失能；
        #     纯力矩模式(kp=kd=0)又没有位置反馈 → 角度完全不可控。
        #     专用通道保留完整位置环，扭矩按"能否跟上目标"自适应并封顶，
        #     角度可控 + 抓得稳 + 不过流失能，三者兼得。
        gripper_angles = {}
            ...
            for _cfg_name, _arm in (("left_arm", "left"), ("right_arm", "right")):
                for _jname, _jinfo in (servo_ids.get(_cfg_name) or {}).items():
                    if isinstance(_jinfo, dict) and _jname.endswith("arm8"):
                        ...
                        if _gid is None:
                            continue
                        # all_pos 里的值就是目标角度(度)，交给驱动层按自适应扭矩下发
                        gripper_angles[_gid] = float(all_pos.pop(_gid, 0.0))
        ...
        # 夹爪目标角度（自适应扭矩通道），同样按真实端口分组
        for port, targets in self._group_by_port(gripper_angles, online_servos).items():
            if targets:
                actions["gripper_commands"].append({"port": port, "targets": targets})
