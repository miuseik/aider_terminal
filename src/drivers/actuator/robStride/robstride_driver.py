...
from src.drivers.actuator.robStride.robstride_dynamics.can_driver import RobstrideCanDriver, _busy_wait_us
from src.drivers.actuator.robStride.robstride_dynamics.slcan_can_driver import SlcanCanDriver
...
from src.drivers.actuator.robStride.robstride_dynamics.data_types import MotorFeedback
from src.drivers.actuator.robStride.robstride_dynamics.utils import rad_to_deg, deg_to_rad
...
class RobStrideOfficialDriver:
    ...
    def _ensure_ready(self, device_id: int) -> bool:
        ...
        电机使能/切模式帧只发不确认（无 ACK、总线无重传），故发完使能序列后
    ...
    def _enable_sequence(self, device_id: int) -> bool:
        """发送使能序列：disable+clear_fault → MIT 模式 → enable → CAN_TIMEOUT=0。

        返回该序列所有帧是否都成功 TX（注意：只是 TX 成功，不等 ACK；
        是否真正使能由 _is_mit_mode 读回确认）。
        """
        # 1. 清除故障 + 失能（保证干净初始状态，退出硬件故障锁存）
