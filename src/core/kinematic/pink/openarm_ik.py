...
class OpenArmXSolver():
    def __init__(
        ...
        # core/kinematic/pink/ → 上 3 层到项目根 → URDF/openArmX/
        urdf_path: str = None
    ):
        if urdf_path is None:
            from .. import _PROJ_ROOT
            urdf_path = os.path.join(_PROJ_ROOT, "URDF", "openArmX", "openarmx_description", "urdf", "robot", "openarmx_robot.urdf")
        print(f"Loading URDF from: {urdf_path}")
        
        # 提取 URDF 文件所在目录以定位资源文件 (meshes)
        # openArmX/ 目录（包含 openarmx_description 文件夹）
        script_dir = os.path.dirname(os.path.dirname(urdf_path))
