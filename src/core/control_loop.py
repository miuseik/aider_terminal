...
import logging
import time
from src.core.kinematic.base_ramp import BaseVelocityRamp
...
from src.core.robot_interface import RobotInterface
...
from src.inputs.base import ControlGoal, ControlMode
...
class ControlLoop:
    ...
    def __init__(self, command_queue: asyncio.Queue, config: TelegripConfig, control_commands_queue: Optional[queue.Queue] = None):
        ...
        # === 底盘速度斜坡 (消除顿挫) ===
        # 斜坡业务逻辑已封装到 src.core.kinematic.base_ramp.BaseVelocityRamp
    ...
    async def setup(self) -> bool:
        ...
        # 提前拉 servo_ids.yaml 覆盖关节限位（不连硬件，失败不阻塞），保证后续 setup_kinematics 构造 IK 时吃到真实限位
        # 注意：fetch_servo_limits 是同步方法，不要 await
        try:
            self.robot_interface.fetch_servo_limits()
        except Exception as e:
            print(f"⚠️ fetch_servo_limits 异常（忽略，保持 (0,0) 兜底）: {e}")
        ...
        if self.config.enable_pybullet:
            try:
                ...
                # 按需导入可视化器类
                from src.robots.aider.visualizer import AiderVisualizer
                from src.robots.aloha.visualizer import AlohaVisualizer
                ...
                if not self.visualizer.setup():
                    print("[ControlLoop] PyBullet 仿真不可用，已切换至真机控制模式")
                    self.visualizer = None
                else:
    ...
    async def _execute_goal(self, goal: ControlGoal):
        ...
        if (arm_state.mode == ControlMode.POSITION_CONTROL and
            ...
            try:
                from scipy.spatial.transform import Rotation as _R
                from src.core.kinematic.utils import vr_rotation_to_robot
        ...
        from src.inputs.base import _ACTIVE_INPUT_SOURCES as _wrist_sources
    ...
    async def _update_robot(self):
        ...
        from src.inputs.base import _ACTIVE_INPUT_SOURCES
    ...
    def status(self) -> Dict:
        ...
        # 录制状态（前端菜单据此渲染列表）
        try:
            from src.core.recorder import list_recordings
    ...
    async def handle_rec_command(self, msg: Dict):
        ...
        from src.core.recorder import Recording, PlaybackProvider, rename_recording, list_recordings
