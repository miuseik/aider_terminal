...
class ControlLoop:
    ...
    def _update_robot(self):
        ...
        # Update left arm (only if connected OR in simulation-only mode)
        if (self.left_arm.mode == ControlMode.POSITION_CONTROL and 
            self.left_arm.target_position is not None):
            # Check if arm is connected or we're in no-robot mode
        ...
        # Update right arm (only if connected OR in simulation-only mode)
        ...
        # Send commands to robot (only if actually connected and engaged)
    ...
    def _update_visualization(self):
        ...
        # In no-robot mode, get_arm_angles returns the simulation angles
