...
from .protocol import (
    CommType, MotorType, RunMode, ParamIndex,
    MOTOR_PARAMS, DEFAULT_MOTOR_TYPE_MAP,
)
from .data_types import (
    MotorFeedback, ParamReadResult, FirmwareVersion,
)
from .utils import float_to_uint16, uint16_to_float

logger = logging.getLogger("robstride.can_driver")
...
class RobstrideCanDriver:
    ...
    def connect(self) -> bool:
        ...
        # ── 启动前检查：tx_queue_len 和总线健康状态 ──
        self._check_can_setup()
        return True

    def _check_can_setup(self):
        """检查 CAN 接口基础配置，对常见问题发出告警（参考 EDULITE_A3 启动流程）"""
        # 1) 检查 tx_queue_len（默认=10，只够装 10 帧，极易被撑爆）
        try:
            with open(f"/sys/class/net/{self.can_name}/tx_queue_len") as f:
                qlen = int(f.read().strip())
            if qlen < 64:
                logger.warning(
                    "CAN TX 队列过小 (tx_queue_len=%d)，容易丢帧！请执行: "
                    "sudo ip link set %s txqueuelen 128",
                    qlen, self.can_name,
                )
            else:
                logger.debug("CAN TX 队列大小: %d", qlen)
        except (FileNotFoundError, PermissionError, OSError):
            logger.debug("无法读取 %s tx_queue_len", self.can_name)

        # 2) 检查总线状态
        state = self.check_bus_health()
        if state == "BUS-OFF":
            logger.warning(
                "CAN 总线 %s 处于 BUS-OFF 状态！请重置总线: "
                "sudo ip link set %s down && sudo ip link set %s up type can bitrate 1000000",
                self.can_name, self.can_name, self.can_name,
            )
        elif state == "ERROR-PASSIVE":
            logger.warning(
                "CAN 总线 %s 处于 ERROR-PASSIVE 状态，可能很快进入 BUS-OFF，请检查接线和终端电阻",
                self.can_name,
            )
        elif state == "STOPPED":
            logger.warning(
                "CAN 接口 %s 已停止，请执行: sudo ip link set %s up", self.can_name, self.can_name,
            )
        else:
            logger.info("CAN 总线 %s 状态: %s", self.can_name, state)
    ...
    def _parse_frame(self, raw: bytes):
        ...
        if comm_type == CommType.FEEDBACK and 1 <= motor_id <= 0xFD:
    ...
    def set_can_id(self, motor_id: int, new_id: int) -> bool:
        """修改电机 CAN ID（Type 7）

        协议帧布局（与 _build_extended_can_id 不同！）：
          bits 28-24: CommType (7)
          bits 23-16: new CAN ID
          bits 15-8:  host CAN ID
          bits 7-0:   current motor CAN ID
        """
        can_id = ((CommType.SET_CAN_ID & 0x1F) << 24) | ((new_id & 0xFF) << 16) | ((self.host_can_id & 0xFF) << 8) | (motor_id & 0xFF)
        can_id |= CAN_EFF_FLAG
        return self._send_frame(can_id, bytes(8))
