...
# URDF 文件位于 sim/urdf/ 目录
URDF_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../../../../aider_go/src", "sim")
...
class FKComputer:
    ...
    def __init__(self, urdf_path: str = URDF_PATH):
        ...
        Args:
            urdf_path: URDF 文件路径，默认为 sim/urdf/aider_pro.SLDASM.urdf
        """
        self.tree = ET.parse(urdf_path)
        self.root = self.tree.getroot()
        self._links: Dict[str, dict] = {}       # 所有连杆 {name: {name}}
        self._joints: Dict[str, dict] = {}      # 所有关节 {name: {type, parent, child, origin, axis, limits}}
        self._parents: Dict[str, str] = {}      # child_link → parent_link 父子关系映射
        self._parse()
    ...
    def _parse(self) -> None:
        ...
        解析后填充 self._links, self._joints, self._parents。
        origin 变换被预计算为 4×4 矩阵 T_origin = translate(xyz) @ RPY。
        """
    ...
    def _forward_chain(self, leaf_link: str, joint_values: Dict[str, float]) -> np.ndarray:
        ...
        从 base_link 正向递推到 leaf_link。
        """
        # 收集路径: leaf → ... → base_link
        path = [leaf_link]
        while path[-1] in self._parents:
            path.append(self._parents[path[-1]])
        path.reverse()  # base_link → leaf_link

        T = np.eye(4)
        for i in range(len(path) - 1):
            parent = path[i]
            child = path[i + 1]
            # 找到连接这两个 link 的 joint
    ...
    def pos(self, leaf_link: str, joint_values: Dict[str, float]) -> Tuple[float, float, float]:
        ...
        return tuple(T[:3, 3].tolist())  # type: ignore[return-value]
    ...
    def end_effectors(
        ...
        返回: {
            "left":  (x, y, z),   # left_arm8  原点
            "right": (x, y, z),   # right_arm8 原点
        }
        """
        result = {}
