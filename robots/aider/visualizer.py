...
class AiderVisualizer:
    ...
    def _can_use_display(self) -> bool:
        ...
        try:
            import subprocess
            result = subprocess.run(['xdpyinfo'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=2)
            if result.returncode != 0:
                return False
            result = subprocess.run(['glxinfo'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=5)
            if result.returncode != 0:
    ...
    def setup(self) -> bool:
        ...
        if use_gui and not self._can_use_display():
            print("No display available, falling back to headless mode")
            use_gui = False
